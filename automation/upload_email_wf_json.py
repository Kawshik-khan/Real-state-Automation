...
const backendNode = node({
  ...
  config: {
    ...
    parameters: {
      method: 'POST',
      url: '={{ $env.FASTAPI_BASE_URL || "http://localhost:8000" }}/api/v1/email/incoming',
      sendHeaders: true,
      specifyHeaders: 'json',
...
const sendEmailNode = node({
  ...
  config: {
    ...
    parameters: {
      method: 'POST',
      url: '={{ $env.FASTAPI_BASE_URL || "http://localhost:8000" }}/api/v1/email/threads/dispatch-outbound',
      sendHeaders: true,
      specifyHeaders: 'json',
